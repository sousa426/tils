...
average_accuracy = []
average_time = []
for loop in range(28,total_iterations):
    ...
    ######################### LER DATASET #########################
    mito_data = pd.read_excel('./dataset/Datasets_ALS.xlsx', sheet_name='Mitostress clean',engine='openpyxl')
    y = mito_data.Target # Target variable
    
    mito_data.drop(['ID','Familiy','N','Data','Description','Target'], axis=1,  inplace=True)
