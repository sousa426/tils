...
size_of_inicial_dataset = 0.2  #escala de [0,1], tipo 0.2
...
values_for_test = [1,3,5,10]
for vft in values_for_test:
    number_features_add = vft    

    
    total_iterations = 30
    average_accuracy = []
    average_time = []
    for loop in range(total_iterations):
        start_time = time.time()
        Path('./logs/'+str(vft)).mkdir(parents=True, exist_ok=True)
        filename = './logs/'+str(vft)+'/loop_'+str(loop)+'_tendency_'+str(tendency)+'_number_features_add_'+str(vft)+'.txt'    
        f = open(filename, "w")
        ...
        ######################### LER DATASET #########################
        mirna = pd.read_excel('./dataset/DataSet_Bioinfo1718.xlsx', sheet_name='Sheet1',engine='openpyxl')
        y = mirna.ID # Target variable
        #garantir que nao escolho aleatoriamente a coluna do target
        mirna = mirna.drop('ID', axis=1)
        
        
        ######################### ALTERAR NOMES COLUNAS #########################
        ##mudar os nomes das features para F1, F2, .... F100
        temp_cols = mirna.columns
        final_cols = ['F' + str(x+1) for x in range(len(temp_cols))]
        mirna.columns = final_cols
        ...
        n_features_ini = round(len(mirna.columns) * size_of_inicial_dataset)
        X = mirna.sample(n=n_features_ini,axis='columns')
        ...
        for x in range(type_of_run_value if type_of_run == 'loop' else max_iteration_loop):
            ...
            if i >= mumber_to_perturbe:
                ...
                for x2 in range(number_features_add):
                    ...
                    oper,collum_value,collum_name,console_name= select_operation_ils(colunas,X,mirna)
                ...
                oper,collum_value,collum_name,console_name,collum_name_new,res_oper_name = select_operation(colunas,X,mirna,last_oper_benefit)
        ...
        print("Features Removed:",[re.sub(r'\b\w+\b', lambda m: dictOfWords.get(m.group(), m.group()), s) for s in cols_new])
        print(resultados)
    ...
    file_name2 = './logs/'+str(vft)+'/resume_tendency_'+str(tendency)+'_number_features_add_'+str(vft)+'.txt'
    f = open(file_name2, "w")
